"""
Transform Widget.
Inspector panel for manipulating spatial coordinates (Position, Rotation, Scale).
"""
...
class TransformWidget(BaseComponentWidget):
    """
    Provides precision numeric inputs for translating, rotating, and scaling the selected entity.
    """
    ...
    def __init__(self, controller: Any) -> None:
        ...
        # Call request_undo_snapshot via Controller to save history before edits
        self.w_pos, self.sp_pos = create_vec3_input(
            "Position:", self.apply_transform, 
            step=TRANSFORM_POS_STEP, 
            press_callback=self.request_undo_snapshot
        )
        self.w_rot, self.sp_rot = create_vec3_input(
            "Rotation:", self.apply_transform, 
            min_val=TRANSFORM_ROT_RANGE[0], max_val=TRANSFORM_ROT_RANGE[1], 
            step=TRANSFORM_ROT_STEP, 
            press_callback=self.request_undo_snapshot
        )
        self.w_scl, self.sp_scl = create_vec3_input(
            "Scale:", self.apply_transform, 
            default=DEFAULT_SPAWN_SCALE[0], min_val=TRANSFORM_SCL_MIN, 
            step=TRANSFORM_SCL_STEP, 
            press_callback=self.request_undo_snapshot
        )
    ...
    def update_data(self, tf_data: Dict[str, Any], has_light: bool, light_type: str) -> None:
        """Update widget visibility and fields based on entity data."""
    ...
    def reset_transform(self) -> None:
        """Trigger resetting the transform to default."""
        if self._controller:
