...
Provides specialized UI components for numerical data entry with robust 
Undo/Redo state tracking and synchronized slider/spinbox interactions.
"""
...
class TrackedSpinBox(QDoubleSpinBox):
    ...
    Standard spinboxes emit 'valueChanged' on every keystroke. This class intercepts 
    focus and click events to emit an 'editingStarted' signal exactly once before 
    the value mutates, allowing the system to record a clean pre-edit state snapshot.
    """
    ...
    def focusInEvent(self, event: QFocusEvent) -> None:
        """Triggers a state snapshot when the user tabs into or clicks the input field."""
    ...
    def mousePressEvent(self, event: QMouseEvent) -> None:
        """Ensures a snapshot is recorded if clicked while unfocused."""
    ...
    def keyPressEvent(self, event: QKeyEvent) -> None:
        """Ensures a snapshot is recorded upon the first keystroke."""
        if not self._has_emitted:
            self.editingStarted.emit()
            self._has_emitted = True
...
class SliderSpinBox(QWidget):
    ...
    Implements careful signal management to prevent recursive feedback loops.
    """
    
    def __init__(self, min_val: float, max_val: float, step: float, default_val: float, 
                 callback: Callable[[], None], press_callback: Optional[Callable[[], None]] = None) -> None:
        super().__init__()
        ...
        # QSlider natively supports only integers. A multiplier is applied for floating-point precision.
        ...
        # Cross-connect the widgets to synchronize states
    ...
    def _sync_spin(self, val: float) -> None:
        """Updates the Slider from the SpinBox while blocking its signals to prevent a feedback loop."""
    ...
    def _sync_slider(self, val: int) -> None:
        """Updates the SpinBox from the Slider while blocking its signals."""
    ...
    def value(self) -> float: 
        """Returns the current numerical value of the composite widget."""
...
def create_vec3_input(label_text: str, callback: Callable[[], None], default: float = 0.0, 
                      min_val: float = GLOBAL_NUMERIC_MIN, max_val: float = GLOBAL_NUMERIC_MAX, 
                      step: float = 0.1, press_callback: Optional[Callable[[], None]] = None) -> Tuple[QWidget, List[TrackedSpinBox]]:
    """
