...
class Renderer(BaseRenderer):
    ...
    def _render_proxies(self, view_matrix: glm.mat4, projection_matrix: glm.mat4, active_camera: CameraComponent) -> None:
        ...
        for tf, mesh, ent in self.queue.proxies:
            ...
            if hasattr(geom_obj, 'draw'): geom_obj.draw()
    ...
    def raycast_select(self, scene: Any, mx: float, my: float, width: int, height: int) -> int:
        if width <= 0 or height <= 0 or mx < 0 or my < 0 or mx >= width or my >= height: return -1
        if self.picking_width != width or self.picking_height != height: self._setup_picking_fbo(width, height)
        ...
        for item_list in [self.queue.opaque, self.queue.transparent, self.queue.proxies]:
            for tf, mesh, ent in item_list:
                if not self._is_entity_globally_visible(ent): continue
                ent_idx = scene.entities.index(ent)
                if ent_idx == cam_idx: continue
        ...
        if pixel_data:
            ...
            if r == 255 and g == 255 and b == 255: return -1
            hit_idx = r + (g << 8) + (b << 16)
            if 0 <= hit_idx < len(scene.entities): return hit_idx
        return -1