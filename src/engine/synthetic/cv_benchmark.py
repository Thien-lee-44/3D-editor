...
class CVBenchmarkConfig:
    ...
    task: str = "auto"  # auto | detect | segment
    epochs: int = 3
    batch_size: int = 8
    imgsz: int = 640
    ...
    max_visualization_frames: int = 0  # 0 = all frames


class CVBenchmarkRunner:
    """
    Runs reproducible CV benchmarks for one or more dataset variants.
    Exports metrics and visual comparison artifacts (GT vs Prediction).
    """
    ...
    def run(
        ...
        for idx, (variant_name, variant_dir_raw) in enumerate(items, start=1):
            ...
            try:
                ...
                class_lookup = self._load_class_names(effective_dataset_yaml)
    ...
    def _write_artifacts(self, records: List[Dict[str, Any]]) -> Dict[str, str]:
                ...
                writer.writerow(
                    {
                        ...
                        "dataset_yaml": rec.get("dataset_yaml", ""),
                        "effective_dataset_yaml": rec.get("effective_dataset_yaml", ""),
                        "task": rec.get("task", ""),
                        "model_type": rec.get("model_type", ""),
                        "num_images": rec.get("num_images", 0),
                        "num_labels": rec.get("num_labels", 0),
                        "primary_metric_name": rec.get("primary_metric_name", ""),
                        ...
                        "comparison_dir": rec.get("comparison_dir", ""),
                        "comparison_csv": rec.get("comparison_csv", ""),
                        "comparison_json": rec.get("comparison_json", ""),
                        "error": rec.get("error", ""),
                    }
                )
        ...
        with open(summary_md, "w", encoding="utf-8") as f:
            ...
            if not ranked:
                ...
                f.write("| Variant | Task | Imgs | Lbls | Primary | Box mAP@50 | Seg mAP@50 | Match P | Match R | Visualized/Total |\n")
                f.write("|---|---|---:|---:|---:|---:|---:|---:|---:|---:|\n")
                for rec in ranked:
                    m = rec.get("metrics", {})
                    f.write(
            ...
            frame_rows.append(
                {
                    ...
                    "precision_like": frame_precision,
                    "recall_like": frame_recall,
                    "comparison_image": str(out_img.name),
                }
            )
            ...
            json.dump(
                {
                    ...
                    "visualized_frames": len(selected_paths),
                    "sum_gt": sum_gt,
                    "sum_pred": sum_pred,
                    "sum_matched": sum_matched,
                    "match_precision": overall_precision,
                ...
                if len(parts) == 5:
                    ...
                    try:
                        for i in range(1, len(parts), 2):
                            nx = float(parts[i])
                            ny = float(parts[i + 1])
                            coords.append((nx * width, ny * height))
    ...
    def _count_matches(
        ...
        for gt in gt_boxes:
            ...
            for j, pred in enumerate(pred_boxes):
                if j in used_pred:
                    continue
                if int(pred.get("class_id", -2)) != gt_cls:
                    continue
    ...
    def _load_class_names(self, dataset_yaml: Path) -> Dict[int, str]:
        ...
        # Fallback parser for minimal YAML structures.
        in_names = False
        try:
            with open(dataset_yaml, "r", encoding="utf-8") as f:
                for raw in f:
    ...
    def _resolve_task(self, dataset_dir: Path) -> str:
        ...
        if self._labels_look_like_segmentation(dataset_dir / "labels"):
            return "segment"
        if self._labels_look_like_segmentation(dataset_dir / "labels_seg"):
            return "segment"
        return "detect"
    ...
    def _resolve_model_type(self, task: str) -> str:
        if self.config.model_type:
            ...
            if task == "detect" and ("-seg" in custom_model or custom_model.endswith("seg.pt")):
                raise ValueError(
                    f"Model '{custom_model}' is segmentation-oriented but task is detect. "
                    "Use task='segment' or select a detection model."
                )
            return custom_model
        if task == "segment":
    ...
    def _prepare_segment_variant(self, dataset_dir: Path, safe_name: str) -> tuple[Path, Path]:
        ...
        if not source_images.exists() or not source_labels.exists() or not source_yaml.exists():
            raise FileNotFoundError(
                f"Missing required files for segmentation preparation in: {dataset_dir}\n"
                "Need images/, labels/, and dataset.yaml"
            )
        ...
        if not self._labels_look_like_segmentation(labels_seg):
            raise ValueError(
                "Cannot run segmentation benchmark: labels are detection format and conversion failed."
            )
        ...
        for image_file in self._iter_images(source_images):
            target = prepared_images / image_file.name
            self._link_or_copy(image_file, target)

        for label_file in sorted(labels_seg.glob("*.txt")):
            shutil.copy2(label_file, prepared_labels / label_file.name)
    ...
    def _build_labels_seg_from_detection(self, source_labels: Path, target_labels_seg: Path) -> None:
        ...
        for det_file in sorted(source_labels.glob("*.txt")):
            seg_file = target_labels_seg / det_file.name
            seg_lines: List[str] = []
    ...
    def _write_prepared_dataset_yaml(self, source_yaml: Path, target_yaml: Path, prepared_dir: Path) -> None:
        ...
        for line in text.splitlines():
            ...
            if stripped.startswith("path:"):
                ...
                out_lines.append("train: images")
                replaced_train = True
            elif stripped.startswith("val:"):
                out_lines.append("val: images")
                replaced_val = True
            else:
                out_lines.append(line)

        if not replaced_path:
            out_lines.insert(0, f"path: {prepared_dir.as_posix()}")
